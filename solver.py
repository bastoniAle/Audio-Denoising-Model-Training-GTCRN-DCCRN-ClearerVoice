...
from losses.loss import loss_frcrn_se_16k, loss_mossformer2_se_48k, loss_mossformergan_se_16k, loss_dccrn_wsdr_fn, loss_sisnr, DPCRNLoss, gtcrnHybridLoss
...
class Solver(object):
    def __init__(self, args, model, optimizer, discriminator, optimizer_disc, train_data, validation_data, test_data):
        ...
        elif self.args.network == 'DCUNET_SE_48K':
            self._run_one_epoch = self._run_one_epoch_dcunet222_se_48k
        elif self.args.network == 'DCCRN_SE_48K':
            self._run_one_epoch = self._run_one_epoch_dccrn_se_48k
        elif self.args.network == 'PHASEN_SE_48K':
            self._run_one_epoch = self._run_one_epoch_phasen_se_48k
        elif self.args.network == 'DPCRN_SE_48K':
            self._run_one_epoch = self._run_one_epoch_dpcrn_se_48k
        elif self.args.network == 'GTCRN_SE_48K':
            self._run_one_epoch = self._run_one_epoch_gtcrn_se_48k
        elif self.args.network == 'ULUNAS_SE_48K':
            self._run_one_epoch = self._run_one_epoch_ulunas_se_48k
        elif self.args.network =='DTLN_SE_48K':
            self._run_one_epoch = self._run_one_epoch_dtln_se_48k
        else:
    ...
    def _run_one_epoch_dcunet222_se_48k(self, data_loader, state='train'):
        if state == 'train':
            self.model.train()
        else:
            self.model.eval()
        ...
        for i, batch in enumerate(data_loader):
            ...
            if state == 'train':
                self.optimizer.zero_grad()
                enhanced, _, _ = self.model(inputs)
                loss = loss_dccrn_wsdr_fn(inputs, enhanced, labels)
                loss.backward()
                torch.nn.utils.clip_grad_norm_(self.model.parameters(), self.args.clip_grad_norm)
                self.optimizer.step()
            else:
            ...
            if (i + 1) % self.args.print_freq == 0:
                ...
                print(f'Train Epoch: {self.epoch}/{self.args.max_epochs}, Step: {i+1}/{num_batches} | '
                      f'lr {lr:.1e} | Loss: {avg_loss:.6f}')
    ...
    def _run_one_epoch_dpcrn_se_48k(self, data_loader, state='train'):
        #TODO
        return 0

    def _run_one_epoch_gtcrn_se_48k(self, data_loader, state='train'):
        #TODO
        return 0

    def _run_one_epoch_ulunas_se_48k(self, data_loader, state='train'):
        #TODO
        return 0

    def _run_one_epoch_dtln_se_48k(self, data_loader, state='train'):
        #TODO
        return 0
