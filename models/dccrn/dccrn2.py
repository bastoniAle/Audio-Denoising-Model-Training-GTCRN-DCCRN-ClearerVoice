...
from models.dccrn.tools_for_model import ConvSTFT, ConviSTFT, ComplexConv2d, ComplexConvTranspose2d, NavieComplexLSTM, complex_cat, ComplexBatchNorm
from models.w_scripts import STFT_W
from models.dccrn.tools_for_loss import si_snr, si_sdr, get_array_mel_loss, pmsqe_stft, pmsqe_loss, sdr
...
class DCCRN(nn.Module):
    ...
    def __init__(
        ...
        self.w_stft = STFT_W(n_fft=self.fft_len, hop_length=self.win_inc, win_length=self.win_len, window=self.win_type)
        self.stft = self.w_stft.stft
        self.istft = self.w_stft.istft
    ...
    def forward(self, inputs, lens=None):
        ...
        out_spec = torch.stack([real, imag], dim=-1).unsqueeze(1)
        out_wav = self.istft(out_spec)

        out_wav = torch.squeeze(out_wav, 1)
        # out_wav = torch.tanh(out_wav)
        out_wav = torch.clamp_(out_wav, -1, 1)
        return mask_real, mask_imag, real, imag, out_wav
...
class DCCRN_SE_48K(nn.Module):
    def __init__(self, args):
        super(DCCRN_SE_48K, self).__init__()
        self.model = DCCRN(win_len=args.win_len, win_inc=args.win_inc, fft_len=args.fft_len, win_type=args.win_type)
    def forward(self, x):
        mask_real, mask_imag, real, imag, out_wav = self.model(x)
